...
class WarehouseIn(models.TransientModel):
    _name = 'warehouse.in'
    ...
    def default_get(self, fields):
        res = super(WarehouseIn, self).default_get(fields)
        srd = self.env['stock.picking.type'].search([('sequence_code', '=', 'IN')])
    ...
    def read_file(self):
        ...
        warehouse_in = self.env['stock.picking'].create({
            'picking_type_id': self.picking_type_id.id,
            'location_id': self.env.ref('stock.stock_location_suppliers').id,
            'location_dest_id': self.env.ref('stock.stock_location_customers').id,
        })
        warehouse_in._onchange_picking_type()
        ...
        for product_id in product_list:
            new_move = self.env['stock.move'].create({
                ...
                'picking_id': warehouse_in.id,
                'location_id': warehouse_in.location_id.id,
                'location_dest_id': warehouse_in.location_dest_id.id,
            })
            new_move._onchange_product_id()

        warehouse_in.action_confirm()
