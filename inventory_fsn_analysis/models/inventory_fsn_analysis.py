...
class InventoryFsnAnalysis(models.TransientModel):
    ...
    fast_moving = fields.Integer(string="Fast Moving", required=True)
    slow_moving = fields.Integer(string="Slow Moving", required=True)
    non_moving = fields.Integer(string="Non Moving", required=True)
    ...
    def default_action(self):
        ...
        action = {
            ...
            'view_mode': 'form',
            'res_id': movement.id,
            'view_id': self.env.ref('inventory_fsn_analysis.inventory_fsn_form_view').id,
    ...
    def calculate_sale(self, product):
        sale = 0.0
        for rec in self:
            stock_movement_lines = self.env['stock.move'].search(
                [('date', '>=', rec.start_date), ('date', '<=', rec.end_date), ('picking_code', '=', 'outgoing'),
                 ('sale_line_id', '!=', False), ('state', '=', 'done'),
                 ('product_id', '=', product.id)], order='date')
    ...
    def create_line(self):
        for rec in self:
            ...
            if not rec.product_ids and not rec.product_category:
                products = self.env['product.product'].search([], order='name')
            else:
                products = self.env['product.product'].search(
                    ['|', ('id', 'in', product_ids.ids), ('categ_id', 'in', product_category.ids)], order='name')
