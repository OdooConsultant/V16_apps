...
class StockMovementReportXlsx(models.AbstractModel):
    _name = 'report.all_in_one_inventory_analysis.report_stock_movement_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, stock):
        ...
        format4 = workbook.add_format({'font_size': 14, 'font_color': 'green', 'align': 'center', 'bold': True})
        format5 = workbook.add_format({'font_size': 14, 'font_color': 'red', 'align': 'center', 'bold': True})
        ...
        for obj in stock:
            ...
            row = 3
            sheet.set_row(3, 25)
            sheet.merge_range('F4:K4', 'Inventory Stock Movement', format1)
            ...
            if obj.product_ids:
                ...
                for item in obj.product_ids:
                    sheet.merge_range(f'J{row_count_pro}:K{row_count_pro}', item.display_name, format8)
                    row_count_pro += 1
                row += 2

            row = row_count_pro + 1 if row_count_pro > row_count_cat else row_count_cat + 1
            sheet.write(row, 6, 'Total Opening', format2)
            sheet.write(row, 7, 'Total In', format2)
            sheet.write(row, 8, 'Total Out', format2)
            sheet.write(row, 9, 'Total Closing', format2)

            row += 1
            sheet.write(row, 6, obj.total_opening_stock, format3)
            sheet.write(row, 7, obj.total_stock_in, format4)
            sheet.write(row, 8, obj.total_stock_out, format5)
            sheet.write(row, 9, obj.total_closing_stock, format3)
            ...
            sheet.write(row, 8, 'Stock In', table_header_left)
            sheet.write(row, 9, 'Stock Out', table_header_left)
            sheet.write(row, 10, 'Closing Stock', table_header_left)
            ...
            for line in obj.sm_lines:
                ...
                sheet.write(row, 8, line.stock_in, format7)
                sheet.write(row, 9, line.stock_out, format7)
                sheet.write(row, 10, line.closing_stock, format7)
                row += 1
